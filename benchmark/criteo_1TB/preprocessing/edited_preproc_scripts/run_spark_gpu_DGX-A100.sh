...
echo "Transforming the train data from day_0 to day_18..."
spark-submit --master $MASTER \
    --driver-memory "${DRIVER_MEMORY}G" \
    --executor-cores $NUM_EXECUTOR_CORES \
    --executor-memory "${EXECUTOR_MEMORY}G" \
    ...
    --input_folder $INPUT_PATH \
    --days 0-18 \
    --output_folder $OUTPUT_PATH/day0_18 \
    --model_size_file $OUTPUT_PATH/model_size.json \
    --model_folder $OUTPUT_PATH/models \
    --write_mode overwrite --low_mem 2>&1 | tee submit_day0_18_log.txt


echo "Transforming the test data in day19..."
spark-submit --master $MASTER \
    --driver-memory "${DRIVER_MEMORY}G" \
    --executor-cores $NUM_EXECUTOR_CORES \
    --executor-memory "${EXECUTOR_MEMORY}G" \
    ...
    --input_folder $INPUT_PATH \
    --days 19-19 \
    --output_folder $OUTPUT_PATH/day19 \
    --output_ordering input \
    --model_folder $OUTPUT_PATH/models \
    --write_mode overwrite --low_mem 2>&1 | tee submit_day19_log.txt

echo "Transforming the test data in day20..."
spark-submit --master $MASTER \
    --driver-memory "${DRIVER_MEMORY}G" \
    --executor-cores $NUM_EXECUTOR_CORES \
    --executor-memory "${EXECUTOR_MEMORY}G" \
    ...
    --input_folder $INPUT_PATH \
    --days 20-20 \
    --output_folder $OUTPUT_PATH/day20 \
    --output_ordering input \
    --model_folder $OUTPUT_PATH/models \
    --write_mode overwrite --low_mem 2>&1 | tee submit_day20_log.txt

echo "Transforming the test data in day21..."
spark-submit --master $MASTER \
    --driver-memory "${DRIVER_MEMORY}G" \
    --executor-cores $NUM_EXECUTOR_CORES \
    --executor-memory "${EXECUTOR_MEMORY}G" \
    ...
    --input_folder $INPUT_PATH \
    --days 21-21 \
    --output_folder $OUTPUT_PATH/day21 \
    --output_ordering input \
    --model_folder $OUTPUT_PATH/models \
    --write_mode overwrite --low_mem 2>&1 | tee submit_day21_log.txt

echo "Transforming the test data in day22..."
spark-submit --master $MASTER \
    --driver-memory "${DRIVER_MEMORY}G" \
    --executor-cores $NUM_EXECUTOR_CORES \
    --executor-memory "${EXECUTOR_MEMORY}G" \
    ...
    --input_folder $INPUT_PATH \
    --days 22-22 \
    --output_folder $OUTPUT_PATH/day22 \
    --output_ordering input \
    --model_folder $OUTPUT_PATH/models \
    --write_mode overwrite --low_mem 2>&1 | tee submit_day22_log.txt

echo "Transforming the test data in day23..."
spark-submit --master $MASTER \
    --driver-memory "${DRIVER_MEMORY}G" \
    --executor-cores $NUM_EXECUTOR_CORES \
    --executor-memory "${EXECUTOR_MEMORY}G" \
    ...
    --input_folder $INPUT_PATH \
    --days 23-23 \
    --output_folder $OUTPUT_PATH/day23 \
    --output_ordering input \
    --model_folder $OUTPUT_PATH/models \
    --write_mode overwrite --low_mem 2>&1 | tee submit_day23_log.txt
