...
hierarchy_df = pd.read_csv("/scratch/maximilian.boether/scrape/index_label_to_hierarchical.csv").drop(
    "category", axis=1
)
supercategory_to_label = {
    supercategory: label for label, supercategory in enumerate(hierarchy_df["supercategory"].unique())
}
hierarchical_label_to_label = {
    hierarchical_label: label for label, hierarchical_label in enumerate(hierarchy_df["hierarchical_label"].unique())
}
