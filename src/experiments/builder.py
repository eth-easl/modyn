    ...
    if model_config['name'].lower() in ['resnet18', 'resnet-18']:
        ...
        if model_config['in_channels'] != 3:
            model.conv1 = torch.nn.Conv2d(
                model_config['in_channels'], 64, kernel_size=(
                    7, 7), stride=(
                    2, 2), padding=(
                    3, 3), bias=False)
...
def make_trainer(
    ...
    if trainer_name == 'defaultTrainer':
        ...
        return UniformSamplingTaskBasedTrainer(
            model,
            criterion,
            optimizer,
            scheduler_factory,
        ...
        return CheaterTaskBasedTrainer(
            model,
            criterion,
            optimizer,
            scheduler_factory,
        ...
        return HighestLossTaskBasedTrainer(
            model,
            criterion,
            optimizer,
            scheduler_factory,
        ...
        return PoolingTaskBasedTrainer(
            model,
            criterion,
            optimizer,
            scheduler_factory,
