...
class TestRandomScorer(TestScorer):
    ...
    def test_create_shuffled_batches(self):
        ...
        df = pd.read_csv(self.STORAGE_LOCATION + '/tests/data/test.csv', header=0)
        df['row_id'] = [10, 11, 12]
        filename1 = self.scorer._data_storage.write_dataset_to_tar(batchname1, df.to_json())
        df['row_id'] = [20, 21, 22]
        filename2 = self.scorer._data_storage.write_dataset_to_tar(batchname2, df.to_json())
        df['row_id'] = [30, 31, 32]
        filename3 = self.scorer._data_storage.write_dataset_to_tar(batchname3, df.to_json())
        df['row_id'] = [40, 41, 42]
        filename4 = self.scorer._data_storage.write_dataset_to_tar(batchname4, df.to_json())
        ...
        self.scorer.create_shuffled_batches(self.scorer.BATCHES_BY_SCORE, self.scorer.ROWS_BY_SCORE, 2, 4)
