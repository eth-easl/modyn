...
class AlibiDriftDetector(DriftDetector):
    ...
    def detect_drift(
        ...
        embeddings_ref = embeddings_ref.detach().cpu().numpy() if isinstance(embeddings_ref, torch.Tensor) else embeddings_ref
        embeddings_cur = embeddings_cur.detach().cpu().numpy() if isinstance(embeddings_cur, torch.Tensor) else embeddings_cur
