...
class FileWatchdog {
 private:
  YAML::Node config_;
  std::unordered_map<int64_t, std::tuple<std::thread, int16_t, std::atomic<bool>*>> file_watcher_processes_;
 ...
 public:
  FileWatchdog(const YAML::Node& config, std::atomic<bool>* stop_file_watchdog)
    ...
    file_watcher_processes_ = std::unordered_map<int64_t, std::tuple<std::thread, int16_t, std::atomic<bool>*>>();
  }
