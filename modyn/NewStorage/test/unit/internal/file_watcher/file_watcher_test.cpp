...
TEST_F(FileWatcherTest, TestSeek) {
  ...
  *sql << "SELECT * FROM files WHERE path = :path", soci::use(file_path);
  ...
  int last_timestamp;
  *sql << "SELECT last_timestamp FROM datasets WHERE dataset_id = :id",
      soci::use(1), soci::into(last_timestamp);
...
TEST_F(FileWatcherTest, TestExtractCheckValidFile) {
  ...
  EXPECT_CALL(filesystem_wrapper, get_modified_time(testing::_))
...
TEST_F(FileWatcherTest, TestFallbackInsertion) {
  ...
  int file_id;
  *sql << "SELECT file_id FROM files WHERE file_id = :id", soci::use(1),
      soci::into(file_id);
  ASSERT_TRUE(sql->got_data());

  *sql << "SELECT file_id FROM files WHERE file_id = :id", soci::use(2),
      soci::into(file_id);

  ASSERT_TRUE(sql->got_data());

  *sql << "SELECT file_id FROM files WHERE file_id = :id", soci::use(3),
      soci::into(file_id);
...
TEST_F(FileWatcherTest, TestHandleFilePaths) {
  ...
  EXPECT_CALL(filesystem_wrapper, get_created_time(testing::_))
      .WillOnce(testing::Return(1000));

  // TODO: Also mock the file wrapper
  ASSERT_NO_THROW(watcher.handle_file_paths(file_paths, ".txt", "MOCK",
                                            &filesystem_wrapper, 0));
}