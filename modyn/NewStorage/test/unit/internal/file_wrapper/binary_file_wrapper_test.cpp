...
TEST(BinaryFileWrapperTest, TestGetNumberOfSamples) {
  ...
  storage::BinaryFileWrapper file_wrapper =
...
TEST(BinaryFileWrapperTest, TestValidateFileExtension) {
  ...
  file_name = "test.txt";
  ASSERT_THROW(
      storage::BinaryFileWrapper file_wrapper2 =
          storage::BinaryFileWrapper(file_name, config, &filesystem_wrapper),
...
TEST(BinaryFileWrapperTest, TestValidateRequestIndices) {
  ...
  EXPECT_CALL(filesystem_wrapper, get(testing::_))
      .WillOnce(testing::Return(new std::vector<unsigned char>{
          '1', '2', '3', '4', '5', '6', '7', '8'}));
  storage::BinaryFileWrapper file_wrapper(file_name, config,
                                          &filesystem_wrapper);
  ASSERT_NO_THROW(file_wrapper.get_sample(0));

  EXPECT_CALL(filesystem_wrapper, get_file_size(testing::_))
      .WillOnce(testing::Return(8));
  storage::BinaryFileWrapper file_wrapper2(file_name, config,
                                           &filesystem_wrapper);
...
TEST(BinaryFileWrapperTest, TestGetLabel) {
  ...
  std::vector<unsigned char> *bytes =
      new std::vector<unsigned char>{1, 2, 3, 4, 5, 6, 7, 8};
  EXPECT_CALL(filesystem_wrapper, get_file_size(testing::_))
      .WillOnce(testing::Return(8));
  EXPECT_CALL(filesystem_wrapper, get(testing::_))
      .WillRepeatedly(testing::Return(bytes));
  storage::BinaryFileWrapper file_wrapper(file_name, config,
                                          &filesystem_wrapper);
