...
void FileWatchdog::stop_file_watcher_process(int64_t dataset_id, bool is_test) {
  if (this->file_watcher_processes_.count(dataset_id) == 1) {
    // Set the stop flag for the FileWatcher process
    std::get<2>(this->file_watcher_processes_[dataset_id]).get()->store(true);
    // Wait for the FileWatcher process to stop
    if (std::get<0>(this->file_watcher_processes_[dataset_id]).joinable()) {
      std::get<0>(this->file_watcher_processes_[dataset_id]).join();
    }
    if (!is_test) {
      ...
      std::unordered_map<int64_t, std::tuple<std::thread, int, std::shared_ptr<std::atomic<bool>>>>::iterator it;
      it = this->file_watcher_processes_.find(dataset_id);
      this->file_watcher_processes_.erase(it);
    }
  } else {
...
void FileWatchdog::watch_file_watcher_processes(StorageDatabaseConnection* storage_database_connection) {  // NOLINT
  ...
  *sql << "SELECT COUNT(dataset_id) FROM datasets", soci::into(number_of_datasets);
  if (number_of_datasets == 0) {
    ...
    for (const auto& pair : this->file_watcher_processes_) {
  ...
  *sql << "SELECT dataset_id FROM datasets", soci::into(dataset_ids);
  ...
  for (const auto& dataset_id : dataset_ids) {
    if (std::get<2>(this->file_watcher_processes_[dataset_id]) == nullptr) {
      // There is no FileWatcher process registered for this dataset. Start one.
      this->start_file_watcher_process(dataset_id, 0);
    } else if (std::get<1>(this->file_watcher_processes_[dataset_id]) > 2) {
      // There have been more than 3 restart attempts for this process. Stop it.
      this->stop_file_watcher_process(dataset_id);
    } else if (!std::get<0>(this->file_watcher_processes_[dataset_id]).joinable()) {
      // The FileWatcher process is not running. Start it.
      this->start_file_watcher_process(dataset_id, std::get<1>(this->file_watcher_processes_[dataset_id]));
      std::get<1>(this->file_watcher_processes_[dataset_id]) += 1;
    }
  }
}

void FileWatchdog::run() {
  StorageDatabaseConnection storage_database_connection = StorageDatabaseConnection(this->config_);
  ...
  for (auto& file_watcher_process : this->file_watcher_processes_) {
