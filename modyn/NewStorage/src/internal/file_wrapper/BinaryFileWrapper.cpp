...
int BinaryFileWrapper::int_from_bytes(unsigned char *begin,
                                      unsigned char *end) {
  int value = 0;
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  value = std::accumulate(
      begin, end, 0, [](int acc, unsigned char x) { return (acc << 8) | x; });
...
int BinaryFileWrapper::get_number_of_samples() {
  return this->file_size / this->record_size;
}
...
void BinaryFileWrapper::validate_file_extension() {
  ...
  if (extension != "bin") {
    throw std::invalid_argument(
        "Binary file wrapper only supports .bin files.");
  }
}

void BinaryFileWrapper::validate_request_indices(int total_samples,
...
BinaryFileWrapper::get_samples(int start, int end) {
...
std::vector<unsigned char> *BinaryFileWrapper::get_sample(int index) {
  ...
  std::vector<unsigned char> *sample =
      new std::vector<unsigned char>(sample_begin, sample_end);
  return sample;
}

std::vector<std::vector<unsigned char>> *
BinaryFileWrapper::get_samples_from_indices(std::vector<int> *indices) {
  this->validate_request_indices(this->get_number_of_samples(), indices);
  std::vector<std::vector<unsigned char>> *samples =
      new std::vector<std::vector<unsigned char>>;
