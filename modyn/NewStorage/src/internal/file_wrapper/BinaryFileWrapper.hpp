...
public:
  BinaryFileWrapper(std::string path, YAML::Node file_wrapper_config,
    ...
    if (!file_wrapper_config["record_size"]) {
      throw std::runtime_error(
          "record_size must be specified in the file wrapper config.");
    }
    this->record_size = file_wrapper_config["record_size"].as<int>();
    if (!file_wrapper_config["label_size"]) {
      throw std::runtime_error(
          "label_size must be specified in the file wrapper config.");
    }
    ...
    if (this->record_size - this->label_size < 1) {
      throw std::runtime_error("Each record must have at least 1 byte of data "
    ...
    if (this->file_size % this->record_size != 0) {
      throw std::runtime_error(
          "File size must be a multiple of the record size.");
    }
  }
