...
int SingleSampleFileWrapper::get_number_of_samples() {
  if (this->path.find(this->file_wrapper_config["file_extension"].as<std::string>()) == std::string::npos) {
    return 0;
  }
  return 1;
}
...
int SingleSampleFileWrapper::get_label(int index) {
  ...
  auto label_path = std::filesystem::path(this->path).replace_extension(label_file_extension);
