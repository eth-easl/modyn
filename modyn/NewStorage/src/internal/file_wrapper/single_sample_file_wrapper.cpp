...
int SingleSampleFileWrapper::get_number_of_samples() {
  if (this->file_path.find(this->file_wrapper_config["file_extension"].as<std::string>()) == std::string::npos) {
    return 0;
  }
  return 1;
}
...
int SingleSampleFileWrapper::get_label(int index) {
  if (get_number_of_samples() == 0) throw std::runtime_error("File has wrong file extension.");
  if (index != 0) throw std::runtime_error("SingleSampleFileWrapper contains only one sample.");
  if (!this->file_wrapper_config["label_file_extension"]) throw std::runtime_error("No label file extension defined.");
  std::string label_file_extension = this->file_wrapper_config["label_file_extension"].as<std::string>();
  auto label_path = std::filesystem::path(this->file_path).replace_extension(label_file_extension);
...
std::vector<unsigned char>* SingleSampleFileWrapper::get_sample(int index) {
  if (get_number_of_samples() == 0) throw std::runtime_error("File has wrong file extension.");
  if (index != 0) throw std::runtime_error("SingleSampleFileWrapper contains only one sample.");
  return this->filesystem_wrapper->get(this->file_path);
}

std::vector<std::vector<unsigned char>>* SingleSampleFileWrapper::get_samples(int start, int end) {
  if (get_number_of_samples() == 0) throw std::runtime_error("File has wrong file extension.");
  if (start != 0 || end != 1) throw std::runtime_error("SingleSampleFileWrapper contains only one sample.");
...
std::vector<std::vector<unsigned char>>* SingleSampleFileWrapper::get_samples_from_indices(std::vector<int>* indices) {
  if (get_number_of_samples() == 0) throw std::runtime_error("File has wrong file extension.");
  if (indices->size() != 1) throw std::runtime_error("SingleSampleFileWrapper contains only one sample.");
  return new std::vector<std::vector<unsigned char>>{*get_sample(0)};
}
...
void SingleSampleFileWrapper::validate_file_extension() {
  ...
  std::string file_extension = this->file_wrapper_config["file_extension"].as<std::string>();
  if (this->file_path.find(file_extension) == std::string::npos) {
