...
class TriggerSample:
        ...
        if retrieval_worker_id < 0 and total_retrieval_workers < 0:
            ...
            for file in os.listdir(trigger_sample_directory):
                if file.startswith(f"{pipeline_id}_{trigger_id}_{partition_id}") and file.endswith(".txt"):
                    with open(os.path.join(trigger_sample_directory, file), "r") as f:
                        samples.extend(tuple(map(float, line.split(":"))) for line in f.readlines())
            return samples
            ...
            if file.startswith(f"{pipeline_id}_{trigger_id}_{partition_id}") and file.endswith(".txt"):
                with open(os.path.join(trigger_sample_directory, file), "r") as f:
                    for line in f.readlines():
                        if current_index >= start_index and current_index < start_index + worker_subset_size:
    ...
    def save_trigger_sample(
        ...
        with open(samples_file, "w", encoding='utf-8') as file:
            file.write("\n".join("{}:{}".format(x[0], x[1]) for x in trigger_samples))
