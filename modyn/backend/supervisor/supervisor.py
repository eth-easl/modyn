...
class Supervisor:
    ...
    def _setup_trigger(self) -> None:
        ...
        trigger_module = dynamic_module_import(
            "modyn.backend.supervisor.internal.triggers"
        )
        self.trigger: Trigger = getattr(trigger_module, trigger_id)(
            self._on_trigger, trigger_config
        )
    ...
    def _validate_training_options(self) -> bool:
        ...
        if strategy not in Supervisor.supported_strategies:
            logger.error(
                f"Unsupported strategy: {strategy}. Supported strategies = {Supervisor.supported_strategies}"
            )
            is_valid = False
        ...
        if strategy == "finetune":
            if (
                ...
                or "limit"
                not in self.pipeline_config["training"]["strategy_config"].keys()
            ):
                logger.warning(
                    "Did not give any explicit limit on finetuning strategy. Assuming no limit."
                )
        ...
        if self.pipeline_config["training"]["initial_pass"]["activated"]:
            ...
            if reference not in ("amount", "timestamp"):
                logger.error(
                    f"Invalid reference for initial pass: {reference} (valid are 'amount' or 'timestamp')"
                )
                is_valid = False
    ...
    def validate_pipeline_config(self) -> bool:
        return (
            self.validate_pipeline_config_schema()
            and self.validate_pipeline_config_content()
        )
