...
class GRPCHandler:
    ...
    def init_storage(self) -> None:
        ...
        storage_address = (
            f"{self.config['storage']['hostname']}:{self.config['storage']['port']}"
        )
    ...
    def dataset_available(self, dataset_id: str) -> bool:
        assert (
            self.connected_to_storage
        ), "Tried to check for dataset availability, but no storage connection."
        response = self.storage.CheckAvailability(
            DatasetAvailableRequest(dataset_id=dataset_id)
        )
