...
TEST_F(CsvFileWrapperTest, TestGetLabel) {
  ...
  ASSERT_THROW(file_wrapper.get_label(invalid_index), storage::utils::ModynException);

  const int64_t negative_index = -1;
  ASSERT_THROW(file_wrapper.get_label(negative_index), storage::utils::ModynException);
}
