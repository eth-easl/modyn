...
TEST(BinaryFileWrapperTest, TestGetNumberOfSamples) {
  ...
  BinaryFileWrapper file_wrapper(file_name, config, filesystem_wrapper);
...
TEST(BinaryFileWrapperTest, TestValidateFileExtension) {
  ...
  const std::shared_ptr<MockFilesystemWrapper> filesystem_wrapper = std::make_shared<MockFilesystemWrapper>();
  EXPECT_CALL(*filesystem_wrapper, get_file_size(testing::_)).WillOnce(testing::Return(8));
  ASSERT_NO_THROW(const BinaryFileWrapper file_wrapper(file_name, config, filesystem_wrapper););
}
