...
def get_sampler_config(submodular: str = "GraphCut"):
...
def _test_select_subm(submodular):
    sampler = RemoteSubmodularDownsamplingStrategy(*get_sampler_config(submodular))
    sample_ids = [1, 2, 3]
...
def _get_selected_samples(submodular, num_of_target_samples, sample_ids, forward_output, target, embedding):
    ...
    sampler = RemoteSubmodularDownsamplingStrategy(
        ...
        0,
        5,
        {"downsampling_ratio": 10 * num_of_target_samples, "submodular_function": submodular},
