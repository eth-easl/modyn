...
def test_target_size():
    strat = RandomPresamplingStrategy(get_config(), get_minimal_modyn_config(), 10, 1000)
    assert strat.get_presampling_target_size(100) == 50
    assert strat.get_presampling_target_size(20) == 10
    assert strat.get_presampling_target_size(19) == 9
    assert strat.get_presampling_target_size(0) == 0

    with pytest.raises(AssertionError):
        strat.get_presampling_target_size(-1)
