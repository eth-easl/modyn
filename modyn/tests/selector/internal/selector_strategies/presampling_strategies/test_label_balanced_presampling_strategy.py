...
def test_counting_query():
    ...
    count = strat.presampling_strategy._get_samples_count_per_label(strat._next_trigger_id, None)
...
def test_counting_query_reset_after_trigger():
    ...
    count = strat.presampling_strategy._get_samples_count_per_label(strat._next_trigger_id, 0)
    assert count == [100, 25, 20, 5]
    strat.trigger()
    ...
    count = strat.presampling_strategy._get_samples_count_per_label(strat._next_trigger_id, 1)
    assert count == [100, 25, 20, 5, 2, 2]

    # now use tail trigger = 2 (so it should consider all the triggers)
    count = strat.presampling_strategy._get_samples_count_per_label(strat._next_trigger_id, 2)
