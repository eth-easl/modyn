...
TEST_F(LocalStorageBackendTest, TestWriteReadSingle) {
  ...
  modyn::selector::local_storage_backend::parse_file(file.c_str(), static_cast<void*>(data_read.c_str()), 0u, 2u, 0u);
...
TEST_F(LocalStorageBackendTest, TestWriteReadAllMulti) {
  // Hardcoded data to write
  const char* files[] = {"test_a.npy", "test_b.npy"};  // NOLINT
  ...
  modyn::selector::local_storage_backend::write_files_impl(files, data_write.c_str(), data_lengths.data(), 2u);
  modyn::selector::local_storage_backend::parse_files_impl(files, static_cast<void*>(data_read.c_str()),
                                                           data_lengths.data(), data_offsets.data(), 2u);
