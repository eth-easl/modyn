...
def test_prepare_training_set(test_get_samples_by_metadata_query, test_get_info_for_training, test_register_training, test__connection_established, test__init_metadata):
    sample_cfg = {
        'selector': {
    ...
    assert servicer.register_training(RegisterTrainingRequest(training_set_size=8, num_workers=1), None).training_id == 0
    ...
    assert set(servicer.get_sample_keys(GetSamplesRequest(training_id=0, training_set_number=0, worker_id=0), None).training_samples_subset) == set(["a", "b", "c", "d", "e", "f", "g", "h"])
