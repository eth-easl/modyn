...
@patch(
    "modyn.backend.supervisor.internal.grpc_handler.grpc_connection_established",
    return_value=True,
)
...
def test_dataset_available(test_connection_established):
    ...
    with patch.object(
        handler.storage,
        "CheckAvailability",
        return_value=DatasetAvailableResponse(available=True),
    ) as avail_method:
    ...
    with patch.object(
        handler.storage,
        "CheckAvailability",
        return_value=DatasetAvailableResponse(available=False),
    ) as avail_method:
