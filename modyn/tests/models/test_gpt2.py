...
def test_gradient_checkpointing():
    class HParamsWithFeatures:
        def __init__(self, enable_flash_attention=True, enable_gradient_checkpointing=True):
            self.enable_gradient_checkpointing = enable_gradient_checkpointing

    hparams = HParamsWithFeatures()
    model = Gpt2(hparams, "cpu", False)
