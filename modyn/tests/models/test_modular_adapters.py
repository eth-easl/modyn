...
from modyn.models import  apply_lora, apply_kadapter
...
def test_apply_lora():
    ...
    modified_model = apply_lora(model.model, target_modules=target_modules, adapter_dim=16, adapter_alpha=32)
    # Verify that LoRA parameters are trainable and others are frozen.
    for name, param in modified_model.model.named_parameters():
        if "lora" in name:
...
def test_model_with_adapters_inference():
    ...
    assert logits.shape[-1] == tokenizer.vocab_size, "Output dimension mismatch."
