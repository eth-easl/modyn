...
# --- Test Suite for Adapters using the MockTransformer ---
...
def test_apply_lora():
    ...
    model.model = apply_lora(model.model, target_modules=target_modules, adapter_dim=16, adapter_alpha=32).model
...
# Test for adapter-enabled inference
def test_model_with_adapters_inference():
    ...
    # Apply LoRA then Kadapter.
    model.model = apply_lora(model.model, target_modules=["c_attn", "c_proj"]).model
    ...
    # Check that the last dimension (logit size) matches the vocabulary size.
...
# Test for Prompt Tuning
def test_apply_prompt_tuning():
    hparams = HParams()
    model = MockTransformer(hparams, hparams.device, hparams.amp)
    params_before = count_trainable_params(model.model)
    model.model = apply_prompt_tuning(model.model, num_virtual_tokens=10).model
...
# Test for Prefix Tuning
def test_apply_prefix_tuning():
    hparams = HParams()
    model = MockTransformer(hparams, hparams.device, hparams.amp)
    params_before = count_trainable_params(model.model)
    model.model = apply_prefix_tuning(model.model, prefix_length=15).model
    params_after = count_trainable_params(model.model)
