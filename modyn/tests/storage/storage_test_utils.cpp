...
YAML::Node StorageTestUtils::get_dummy_file_wrapper_config_unlabeled(const std::string& byteorder) {
  YAML::Node config;
  config["byteorder"] = byteorder;
  config["file_extension"] = ".txt";
  config["label_file_extension"] = ".json";
...
std::string StorageTestUtils::get_dummy_file_wrapper_config_inline_unlabeled(const std::string& byteorder) {
  std::string test_config = R"(
byteorder: ")" + byteorder + R"("
file_extension: ".txt"
label_file_extension: ".lbl"
