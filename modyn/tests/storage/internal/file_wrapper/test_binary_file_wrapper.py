...
FILE_PATH = str(
    pathlib.Path(os.path.abspath(__file__)).parent / "test_tmp" / "modyn" / "test.bin"
)
FILE_DATA = b"\x00\x01\x00\x02\x00\x01\x00\x0f\x00\x00\x07\xd0"  # [1,2,1,15,0,2000]
INVALID_FILE_EXTENSION_PATH = str(
    pathlib.Path(os.path.abspath(__file__)).parent / "test_tmp" / "modyn" / "test.txt"
)
FILE_WRAPPER_CONFIG = {
    "record_size": 4,
    "label_offset": 0,
...
def test_init():
    file_wrapper = BinaryFileWrapper(
        FILE_PATH, FILE_WRAPPER_CONFIG, MockFileSystemWrapper(FILE_PATH)
    )
