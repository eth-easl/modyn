...
from .pipeline_config import arxiv_bytes_parser_function, arxiv_evaluation_transformer_function
    ...
    21: Experiment(
        ...
        drift_detection_triggers={
            f"{criterion_name}_int{detection_interval}_win{window_size}": DataDriftTriggerConfig(
                ...
                sample_size=5_000,
                metrics={"mmd": AlibiDetectMmdDriftMetric(decision_criterion=criterion, device="cpu")},
            )
            ...
            for criterion_name, criterion in (
                {
                    ...
                    for quantile in [0.05, 0.10, 0.15, 0.3]
                }
                # |
                # {
                ...
                #     )
                #     for deviation in [0.2, 0.5, 1.0, 2.0, 5.0]
                # }
            ).items()
        },
        gpu_device="cuda:3",
