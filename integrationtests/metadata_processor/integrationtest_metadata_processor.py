...
from modyn.backend.metadata_processor.internal.grpc.generated.metadata_processor_pb2 import (
	TrainingMetadataRequest, TrainingMetadataResponse
)
...
def get_grpc_channel(config: dict, component: str) -> grpc.Channel:
	...
	if not grpc_connection_established(processor_channel):
		assert False, f"Could not establish gRPC connection to {component} at {address}"


def send_metadata_and_check_database(processor_client: MetadataProcessorClient, database_client: MetadataDatabaseClient) -> None:
	training_id = 10
	data = {
		"sample1": "metadata01",
		"sample2": "metadata02",
	...
	resp = processor_channel.send_metadata(training_id, serialized_data)
	...
	assert len(database_resp.keys) == len(data), f"Metadata database sent back {len(database_resp.keys)} keys, expected {len(data)}"
	assert len(database_resp.data) == len(data), f"Metadata database sent back {len(database_resp.data)} metadata values, expected {len(data)}"
	assert len(database_resp.seen) == len(data), f"Metadata database sent back {len(database_resp.seen)} seen flags, expected {len(data)}"

	assert database_resp.keys == list(data.keys()), f"Metadata database sent back keys: {str(database_resp.keys)}, expected: {str(list(data.keys()))}"
	assert database_resp.seen == [True] * len(data), f"Metadata database sent back seen flags: {str(database_resp.seen)}, expected all values to be True"
	assert database_resp.data == list(data.values()), f"Metadata database sent back metadata values: {str(database_resp.data)}, expected: {str(list(data.values()))}"
